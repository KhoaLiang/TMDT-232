...
) as combined join belong on belong.bookID=combined.bookID join category on category.id=belong.categoryID group by category.name) as superCombined on superCombined.name=category.name where category.name like ? order by totalSold desc,name limit 5;');
                  if (!$stmt) {
                        http_response_code(500);
...
union
select bookID,count(*) as totalSold from fileOrderContain join book on book.id=fileOrderContain.bookID and book.status=true join customerOrder on customerOrder.id=fileOrderContain.orderID where customerOrder.status=true and week(purchaseTime,1)=week(curdate(),1) group by bookID
) as combined join belong on belong.bookID=combined.bookID join category on category.id=belong.categoryID group by category.name) as superCombined on superCombined.name=category.name where category.name like ? order by totalSold desc,name limit 5;` preparation failed!']);
                        $conn->close();
                        exit;
                  }
